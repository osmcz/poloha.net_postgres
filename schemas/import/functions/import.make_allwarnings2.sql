...
if not found then
	...
	execute insertwarning(v_rec.ruianid,NULL::real,'Adresní bod je pøipojen k nìjaké cestì'::text);
end loop;
close warn_naceste;
...
if v_rec.am_so_distance > 50 then
	execute insertwarning(v_rec.kod,NULL::real,'AM bylo pùvodnì v RUIAN vzdáleno od SO '::text || v_rec.am_so_distance::text || ' metrù. Adresní bod byl posunut k SO'::text);
end if;
end loop;
close warn_am_so;
...
execute insertwarning(v_rec.ruianid,v_rec.distance::real,'Rudý duch - definièní bod budovy je moc daleko od vlastní budovy');
end loop;
close rudyduch;
END;
$$;
