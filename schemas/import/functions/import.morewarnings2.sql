...
--
-- Name: morewarnings2(boolean); Type: FUNCTION; Schema: import; Owner: pedro
--

CREATE FUNCTION morewarnings2(update_mode boolean) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
...
select into v_osmid,v_distance,v_co id,distance_meters(v_rec.pargeom,geom),co from 
	(
	select id,geom,co from osm_adresy order by geom <-> v_rec.pargeom limit 20
	) as foo
		where
	...
	order by st_distance(v_rec.pargeom,geom) limit 1;
--v_go = true;
--if update_mode then
...
ALTER FUNCTION import.morewarnings2(update_mode boolean) OWNER TO pedro;
