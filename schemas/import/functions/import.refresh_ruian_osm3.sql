...
--
-- Name: refresh_ruian_osm3(boolean); Type: FUNCTION; Schema: import; Owner: import
--

CREATE FUNCTION refresh_ruian_osm3(update_mode boolean) RETURNS void
...
insert into ruian_osm_delete (co,osmid,ruianid,deleteit,reallydelete) select co,osmid,ruianid,true,true from ruian_osm where co != 'node';
...
update ruian_osm_delete set deleteit=false where co::text||osmid::text in (
	...
	where r.id is not NULL) and not reallydelete;
...
ALTER FUNCTION import.refresh_ruian_osm3(update_mode boolean) OWNER TO import;

--
-- Name: FUNCTION refresh_ruian_osm3(update_mode boolean); Type: COMMENT; Schema: import; Owner: import
--

COMMENT ON FUNCTION refresh_ruian_osm3(update_mode boolean) IS 'Tato verze by mìla dávat adresy jen na body a ostatní zlikvidovat';
