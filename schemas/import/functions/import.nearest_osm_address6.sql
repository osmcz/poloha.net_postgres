...
--
-- Name: nearest_osm_address6(bigint); Type: FUNCTION; Schema: import; Owner: import
--

CREATE FUNCTION nearest_osm_address6(v_id bigint) RETURNS osmid
...
-- 6.) a nakonec si vystacime se shodou alespon jednoho z cisel, ale vyzadujeme, aby to bylo do 3 metru
v_maxdistance=6;
select into v_co,v_osmid co,id from
    ...
    (substring(housenumber from '[0-9]+') = v_cislo_popisne::text or substring(housenumber from '[0-9]+') = v_cislo_evidencni::text) or 
alternatenumber = v_cislo_popisne::text or substring(alternatenumber from '[0-9]+') = v_cislo_evidencni::text) or 
...
ALTER FUNCTION import.nearest_osm_address6(v_id bigint) OWNER TO import;
