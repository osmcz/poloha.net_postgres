...
--
-- Name: make_changeset2(boolean); Type: FUNCTION; Schema: import; Owner: import
--

CREATE FUNCTION make_changeset2(update_mode boolean) RETURNS void
...
kurzor cursor for select pg.co,pg.kod,pg.id,pg.cislo_popisne,pg.cislo_evidencni,pg.cislo_orientacni,
    pg.cislo_domu,pg.ulice,pg.cast_obce,pg.mestska_cast,pg.obec,pg.psc,
    pg.autocreate,pg.source,pg.sourceaddr,pg.refruian,pg.visible,pg.version,pg.is_in,pg.kod_obce,pg.kod_momc,
    pg.pargeom,pg.kdesevzal
    from parovane_geom pg
    where autocreate;
kurzor2 cursor for select distinct on (co,id) co,id from (
	select co,id from morewarnings union (select distinct on (way_id) 'way'::import.co,way_id as id from osm.current_way_nodes where node_id in (select osmid from ruian_osm where naceste))
	) foo
	where co::text || id::text not in (select co::text || osmid::text from ruian_osm);
kurzor3 cursor for
	select id,co from (
...
fetch kurzor into v_co,v_ruianid,v_osmid,
    ...
    v_street,v_place,v_suburb,v_city,v_postcode,
    v_autocreate,v_source,v_sourceaddr,v_refruian,v_visible,v_version,v_isin,v_kod_obce,v_kod_momc,
    v_pargeom,v_kdesevzal;
if not found then
    exit;
end if;
xml_tags=NULL;
...
if (v_kdesevzal >=4 and v_kdesevzal <=8) or v_kdesevzal = 10 then
	v_zpracovat=true;
end if;
...
if v_co is not NULL then
	insert into inchangeset values (v_co,v_osmid);
end if;
...
when v_co='node' or v_co is NULL then
	...
	v_visible 
        ...
        st_y(st_transform(v_pargeom,4326)) as lat,
        st_x(st_transform(v_pargeom,4326)) as lon
        ),
        xmlelement(
...
then
    ...
    (select node_id from osm.current_way_nodes where way_id = v_osmid) and id not in (select id from inchangeset where co='node') loop
    insert into text_changeset(xml) values(
...
ALTER FUNCTION import.make_changeset2(update_mode boolean) OWNER TO import;
