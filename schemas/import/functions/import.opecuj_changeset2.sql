...
--
-- Name: opecuj_changeset2(boolean); Type: FUNCTION; Schema: import; Owner: pedro
--

CREATE FUNCTION opecuj_changeset2(update_mode boolean) RETURNS void
...
warnings.distance 
    ...
    and
    warnings.distance < 5;
kurzor2 cursor for select * from ruian_osm where autocreate;
BEGIN
...
insert into warnings select v_rec.ruianid,ruianid,distance_meters(v_rec.geom,foo.geom) from (
select * from ruian_osm where ruianid != v_rec.ruianid order by v_rec.geom <-> geom limit 20
...
ALTER FUNCTION import.opecuj_changeset2(update_mode boolean) OWNER TO pedro;
